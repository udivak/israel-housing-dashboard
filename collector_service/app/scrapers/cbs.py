...
class CBSScraper(BaseScraper):
    ...
    Fetches three monthly/bi-monthly time-series from the CBS public price-index API:
    ...
    API docs: https://www.cbs.gov.il/he/cbsNewBrand/Pages/מדדי-מחירים-באמצעות-API.aspx
    Data endpoint: https://api.cbs.gov.il/index/data/price
    Pagination: follow paging.next_url until null (page size configurable, max 100).
    ...
    async def _fetch_all_pages(
        ...
        """Fetch each series ID independently to avoid the CBS server 500 bug.

        The CBS API crashes on page 10+ when multiple series IDs are requested
        in a single combined call.  Fetching one series at a time keeps each
        series within its own pagination window (max 6 pages) and avoids the
        bug entirely.  A failure on one series is logged and skipped so the
        remaining series are still collected.
        """
        all_records: list[dict[str, Any]] = []
        series_ids = [s.strip() for s in self.settings.CBS_SERIES_IDS.split(",") if s.strip()]
    ...
    async def _fetch_series_pages(
        ...
        Transient 500s (especially on page 1) are retried up to
        _CBS_500_MAX_RETRIES times with exponential back-off before giving up.
        Deep-pagination 500s on page >= _CBS_500_DEEP_PAGE are treated as the
        known CBS server bug (all data already fetched) and stop pagination
        without retrying, to avoid wasting time.
        """
        _CBS_500_MAX_RETRIES = 3
        _CBS_500_DEEP_PAGE = 8  # pages beyond this are the known CBS multi-series bug
        ...
        if self.settings.CBS_START_PERIOD:
            params["startPeriod"] = self.settings.CBS_START_PERIOD
        ...
        while url:
            page += 1
            await self._logger.adebug("Fetching CBS page", series_id=series_id, page=page, url=url)
            ...
            if response.status_code == 500:
                await self._logger.awarning(
                    "CBS API returned 500 on pagination page — stopping series early",
                    series_id=series_id,
                    page=page,
                    url=url,
                    records_collected=len(records),
                    retries_exhausted=(retries >= _CBS_500_MAX_RETRIES),
                )
                break
    ...
    def _parse_page(self, data: dict[str, Any]) -> list[dict[str, Any]]:
        """Flatten the CBS response into one record per (series, period) data point."""
        records: list[dict[str, Any]] = []
        ...
        for period_type, series_list in (
            ...
            for series in series_list:
                ...
                for entry in series.get("date") or []:
                    flat = self._flatten_entry(
                        entry, series_code, series_name, period_type
                    )
    ...
    def _flatten_entry(
        ...
        """Produce a flat dict from a single CBS date-entry, ready for normalize_row."""
    ...
    def _encode_params(params: dict[str, Any]) -> str:
        """URL-encode parameters, keeping comma-separated IDs intact."""
