  ...
  return (
    <div  className="flex flex-col gap-4">
      <div>
        ...
        <p className="text-sm text-zinc-400">System status, models, and data sources.</p>
      </div>
      ...
      <Section title="System Health" icon={Activity}>
        <div className="space-y-2 text-xs">
          <div className="flex items-center justify-between rounded-md border border-white/10 bg-zinc-950/40 px-3 py-2">
            ...
            <span className="text-zinc-500">{health.isLoading ? "Checking…" : health.data?.status ?? "DOWN"}</span>
          </div>
...
# or via .env: CHAMPION_MODEL=moses/stacked_v2`}
      ...
      <Section title="Data Sources — Freshness" icon={Database}>
        {sources.length === 0 ? (
            ...
            {sources.map((s) => (
              ...
              >
                ...
                <div className="flex items-center gap-3 text-zinc-500">
                  <span>{s.count.toLocaleString("en-US")} transactions</span>
                  <span>
                    {s.max_date
                      ? new Date(s.max_date).toLocaleDateString("en-US")
                      : "—"}
                  </span>
                </div>
              </div>
      ...
      <Section title="Preferences" icon={RefreshCw}>
        <div className="flex flex-col gap-2 text-xs">
          <div className="flex items-center justify-between rounded-md border border-white/10 bg-zinc-950/40 px-3 py-2">
            <span className="text-zinc-300">Active filters</span>
            <span className="text-zinc-500">{filterCount} fields</span>
          </div>
          <button
